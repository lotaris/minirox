...
(function() {
	 ...
	 */
	MiniRox.ProjectSummaryCollection = Backbone.Collection.extend({
		/**
		 * Initialize
		 *
	 ...
	 */
	MiniRox.ProjectModel = Backbone.Model.extend();

	/**
	 * Represent a ProjectCollection used to manage the different projects
	 * with the tests results associated.
	 */
	MiniRox.ProjectCollection = Backbone.Collection.extend({
		model: MiniRox.ProjectModel,
	 ...
	 */
	MiniRox.TestModel = Backbone.Model.extend({
	 ...
	 */
	MiniRox.TestCollection = GenericTestCollection.extend({
		/**
	 ...
	 * in the details view
	 */
	MiniRox.TestDetailsCollection = GenericTestCollection.extend({
		/**
...
(function() {
	 ...
	 */
	window.MiniRox.app = new (Marionette.Application.extend({
		/**
		...
		projects: new MiniRox.ProjectCollection(),
		summary: new MiniRox.ProjectSummaryCollection(),
		...
		addOrUpdateTest: function(key, coordinates, testAttributes) {
			...
			if (_.isUndefined(this.projects.get(coordinates).get('tests').get(key))) {
				testModel = new MiniRox.TestModel(_.extend(testAttributes, {id: key}));
		...
		addProjectFromTestResult: function(coordinates, name, version, category, testKey) {
			...
			if (_.isUndefined(this.projects.get(coordinates))) {
				...
				var testCollection = new MiniRox.TestCollection({}, {externalState: this.externalState});
				...
				this.projects.add(new MiniRox.ProjectModel({
					id: coordinates,
	...
	MiniRox.app.addInitializer(function(options) {
		...
		this.details = new MiniRox.TestDetailsCollection({}, {externalState: this.externalState});
		...
		this.on('test:run:end', function(data) {
//			this.refreshAll();
		});
...
(function() {
	 ...
	 */
	MiniRox.app.addInitializer(function() {
		var self = this;

		// Connect to MiniROX Server Agent
		this.socket = io.connect(window.location.protocol + '//' + window.location.host);

		// Define strings
		var mini = 'MiniROX Server Agent';
		...
		// Listen when SocketIO client try to reconnect to minirox agent
...
(function() {
	 ...
	 */
	MiniRox.TooltipableView = Marionette.ItemView.extend({
...
(function() {
	 ...
	 */
	var ToolbarView = MiniRox.TooltipableView.extend({
		/**
...
(function() {
	 ...
	 */
	MiniRox.NotificationContainerView = MiniRox.TooltipableView.extend({
...
(function() {
	 ...
	 */
	MiniRox.NotificationView = Marionette.ItemView.extend({
...
(function() {
	 ...
	 * run finished
	 */
	MiniRox.StatisticsNotificationView = MiniRox.NotificationView.extend({
		// Additional class
...
(function() {
	 ...
	 * has occurred.
	 */
	MiniRox.MessageNotificationView = MiniRox.NotificationView.extend({
		// Additional class
		 ...
		 */
		serializeData: function() {
			return _.extend(MiniRox.NotificationView.prototype.serializeData.apply(this), {type: this.options.type});
		}
	});
}).call(this);
(function() {
	 ...
	 * Allows to
	 */
	MiniRox.ReconnectNotificationView = MiniRox.MessageNotificationView.extend({
		// Define
		ui: {
			countdown: '.countdown'
		},
...
(function() {
	 ...
	 * has occurred.
	 */
	MiniRox.TestRunNotificationView = MiniRox.NotificationView.extend({
		// Additional class
...
(function() {
	 ...
	 * has occurred.
	 */
	MiniRox.TestNotificationView = MiniRox.NotificationView.extend({
		// Additional class
		additionalClassName: 'alert-info',
...
//		collection: MiniRox.ProjectCollection,
		gridRegions: {
			table: {
	...
	MiniRox.app.addInitializer(function(options) {
		...
		this.summaryRegion.show(new SummaryTableGridLayout({collection: MiniRox.app.summary}));
	});
}).call(this);
(function() {
	/**
